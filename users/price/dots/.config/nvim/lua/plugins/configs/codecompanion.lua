return {
    {
        ...
        config = function()
            -- From https://gist.github.com/ernie/e8f3a4bb2a01d3f449ec000605631eb8, thanks!
...
All input is written in Typst's math format.
                ...
                adapters = {
                    http = {
                        openrouter = require("codecompanion.adapters").extend(openrouter_http_adapter, {
                            env = {
                                -- GPG is used here instead of just pulling in an environment
                                -- variable so the api key doesn't leak into the environment of
                                -- child processes. E.g. opening Neovim's terminal will not leak the
                                -- actual contents of the API key out to other processes.
                                api_key = ("cmd: gpg --batch --quiet --decrypt '%s'"):format(
                                    vim.env.OPENROUTER_API_KEY_FILE
                                ),
                            },
                            schema = {
                                ["reasoning.effort"] = {
